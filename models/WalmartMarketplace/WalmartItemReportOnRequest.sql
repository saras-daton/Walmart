{% if var('WalmartItemReportOnRequest') %}
{{ config( enabled = True ) }}
{% else %}
...
{{set_table_name('%walmart%itemreportonrequest')}}    
{% endset %}
...
{% for i in results_list %}
    ...
    FROM (
        ...
        FROM  {{i}} a
            {% if var('currency_conversion_flag') %}
            left join {{ref('ExchangeRates')}} c on date(a.ReportDateTime) = c.date and a.CURRENCY = c.to_currency_code
            {% endif %}
            {% if is_incremental() %}
